...
if __name__ == '__main__':
    ...
    for data in data_loc:
        ...
        for count, f in enumerate(filenames): # Analyse in reverse so that the crop images are of the steady state
            ...
            # time has been picked out manually as a good time average before the centreline starts to move
            elif (count == len(filenames) - 1) or (img.time == 327): 

                image_time_ave = plume_time_ave(img, count, img_ave = image_time_ave, thres = plume_absorbance_thres)
                with open(f'{img.img_loc}analysis/plume_time_ave/{str(img.time)}_secs.pickle','wb') as pickle_out:
