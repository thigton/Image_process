...
if __name__ == '__main__':
    ...
    # 2. initial conditions find first instance of interface going down
    # breakpoint()
    h_0 = grad.iloc[0]
    t_0 = grad.index[0]
    def tidy_eff_a(eff_a):
        if eff_a < 0:
            return np.nan
        else:
            return eff_a
    eff_a = (2*S)/(g_ss**0.5*(grad.index- t_0))  * ((0.3*h_0)**0.5 - (0.3*grad)**0.5)
    # breakpoint()
    eff_a = eff_a.apply(tidy_eff_a)
    # breakpoint()


    cd_raw = eff_a / (2**0.5 * bottom_opening_area)

    cd_raw_for_lin_reg = cd_raw[cd_raw.index > 150].dropna()
